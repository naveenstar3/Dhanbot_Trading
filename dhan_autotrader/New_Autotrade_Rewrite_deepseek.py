...
def fetch_candles(security_id, count=None, dhan=None, exchange_segment=None, instrument=None, interval=None):
    try:
        if count:
            # Use default historical API
            from_dt = (datetime.now() - timedelta(minutes=5 * count)).strftime("%Y-%m-%dT%H:%M:%S")
            to_dt = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")

            response = dhanhq.get_historical_candle_data(
            ...
            # This block is used for intraday fetch with dhan object
            now = datetime.now()
            from_dt = now.replace(hour=9, minute=15, second=0, microsecond=0)
            to_dt = now.replace(second=0, microsecond=0)

            response = dhan.get_historical_candle_data(
                security_id=security_id,
...
def main():
    ...
    candle_cache = {}  # Shared cache for all fetches
    
    # 1. Monitor existing positions
    monitor_hold_position()
    if has_hold():
        print("⏩ Active hold exists")
        return
        
    # 2. Load capital and data
    capital = get_capital()
    ...
    # Load master CSV for indices
    master_df = pd.read_csv(MASTER_CSV)
    print("🧪 Master CSV Columns:", master_df.columns.tolist())
    print("🔍 Sample rows:\n", master_df.head(5))
    
    print('📊 Loaded master CSV for index checks')
    
    # 3. Sector mapping setup
    sector_indices = {
        "BANK": "NIFTY BANK", 
        "IT": "NIFTY IT",
        "AUTO": "NIFTY AUTO",
    ...
    # 4. Check market bullishness using NIFTY index
    print("🔍 Searching for NIFTY in master_df...")
    nifty_rows = master_df[
        master_df["SM_SYMBOL_NAME"].str.upper().str.contains("NIFTY") &
        ~master_df["SM_SYMBOL_NAME"].str.upper().str.contains("ETF")
    ]
    
    nifty50_row = nifty_rows.head(1)
    if nifty50_row.empty:
        print("❌ NIFTY index (non-ETF) not found in master")
        send_telegram("❌ CRITICAL: NIFTY index (non-ETF) not found in master")
        return
    ...
    if not nifty_bullish:
        print("📉 Market sentiment weak (NIFTY check failed). Proceeding with sector/stock-level checks.")
        send_telegram("⚠️ NIFTY not bullish. Proceeding with sector/stock-level checks.")
    
    # 5. Multi-stock attempt loop
    order_placed = False
    for index, row in df.iterrows():
        try:
            ...
            candles = fetch_candles(secid, count=30)
            if not candles:
            ...
            if sector in sector_indices:
                sector_index_name = sector_indices[sector]
                sector_row = master_df[master_df["SM_SYMBOL_NAME"] == sector_index_name].head(1)
                if not sector_row.empty:
                    sector_id = sector_row.iloc[0]["SEM_SMST_SECURITY_ID"]
                    if not is_index_bullish(sector_id):
                        print(f'📉 Sector {sector} ({sector_index_name}) not bullish, skipping {symbol}')
                        continue
            ...
            # Time-based trading window check
            market_close = dtime(15, 30)
            ...
            # Position sizing with pattern score and confidence
            price = closes.iloc[-1]
