...
class DoubleBreakEngine:
    ...
    def process_symbol(self, sym: str, sid: str):
        ...
        # grab the candle’s own timestamp instead of now()
        ...
        if st.pulled_back and not st.double_break_complete:
            if st.first_break_side == "LONG" and close > st.high:
                log.info(f"✅✅ {sym}: second LONG break detected @ {close}")
                ...
                log.info(f"✅✅ {sym}: second SHORT break detected @ {close}")
        ...
        while True:
            ...
            try:
                ...
                if all(st.locked for st in self.state.values()):
                    ...
                    pending_symbols = [
                        (s, i)
                        for i, (s, _) in enumerate(self.watch, 1)
                    ...
                    for idx, (sym, sid) in enumerate(self.watch, 1):
                        ...
                        pending_idx = next(
                            (i for i, (s, _) in enumerate(pending_symbols, 1) if s == sym),
                            None
                        )
